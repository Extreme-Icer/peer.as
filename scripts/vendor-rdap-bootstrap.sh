#!/usr/bin/env bash
# 下载 IANA RFC 9224 RDAP bootstrap 表(asn/ipv4/ipv6), 精简合并成前端内置 JSON。
# 前端 rdap.js 据此把 ASN/IP 直接映射到对应 RIR 的 RDAP base, 无需运行时依赖 rdap.org。
...
res = {}
for kind, fn in (("asn","asn.json"), ("ipv4","ipv4.json"), ("ipv6","ipv6.json")):
...
print("wrote", out, "asn:", len(res["asn"]), "ipv4:", len(res["ipv4"]), "ipv6:", len(res["ipv6"]))
PY
