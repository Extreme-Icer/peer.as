...
- **解析(`dns.js resolveDns`)**：纯前端 **DoH JSON**(Cloudflare `cloudflare-dns.com/dns-query`, `Accept:application/dns-json`
  —— Accept 是 CORS 安全列表头, 简单 GET 无预检, 端点回 `ACAO:*`)。并发查 A/AAAA/CNAME/NS/MX/TXT/SOA/CAA 八类,
  单类失败不阻断; 按记录类型 code 过滤 Answer。缓存 = 内存去重 + sessionStorage(`dns:<域名>`)。
...
- **右侧域名详情(`DomainDetail`, `detailKind==='domain'`, 载荷 `S.domainView`)**：逻辑同 ASN 面板——头部 + 概要
  ...
  (注册/到期/变更)/nameservers/DNSSEC。**注意**：部分 TLD(如 `.ie`/`.cn`/`.io`/`.jp`)不在 IANA bootstrap 也不被
  rdap.org 覆盖 → 域名 WHOIS 返回 404/报错(面板优雅显示错误), 但 DNS 记录解析不受影响。
