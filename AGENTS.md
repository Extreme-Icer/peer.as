...
- **`web/`** — 前端 = **Vite + Svelte 5 项目**(不再是裸 JS)。`src/App.svelte` + `src/components/*`(Sidebar/
  ...
  + `src/lib/*`(store.svelte.js 全局 runes 状态、db.js DuckDB-WASM、queries.js 搜索/insight/**asn 视图+面板导航**、
  bgp.js、i18n.js、icons.js Font Awesome、ui.js + **rdap.js**(RDAP 直连)、**rdap-bootstrap.json**(内置 IANA 表)、
...
- **ASN 详情视图**(`AsnDetail.svelte`, `S.detailKind='asn'` + `S.asnView`)：精确框输入 ASN(`classifyQuery` kind=asn)
  ...
  (路由图是 per-prefix 的)。
- **面板导航**(`S.nav={stack,idx}` + `S.subject`)：前进/后退按钮在 ASN↔prefix 间走历史(`navBack/navForward`)；
  prefix 详情里 origin ASN 可点 → `showAsn`。**关闭语义**：`closeInsight` 当看 prefix 且主体是 ASN 时**先返回该 ASN 页**,
  再点才真关；**Esc=`hardCloseDetail` 直接全关**。`showInsight/showAsn` 默认 push 历史(`{push:false}` 供导航复用)。
- **中国访问**：RDAP 在境外, `cn.peer.as` 境内直连可能慢。`rdap.js` 留了 `cnProxy()` 钩子(返回同源 `/rdap/*` 反代 base
  即启用), 待在 Caddy 配反代后接上。
